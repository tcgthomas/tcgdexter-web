...
export async function generateMetadata({
  ...
  const datePart = `Profiled ${new Date(profiledAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}`;
...
/* ─── Collapsible section (client island) ──────────────────── */
...
function CollapsibleSection({
  ...
  // Server component can't use useState — use <details> for zero-JS collapse
  return (
    ...
    return (
      <div className="min-h-screen flex flex-col">
        <header className="flex-shrink-0 pt-12 pb-8 px-6 text-center">
          ...
          <p className="mt-3 text-sm text-brown-500 max-w-md mx-auto leading-relaxed">
            This shared deck link is invalid or has expired.
          </p>
          <Link
            href="/deck-profiler"
            className="inline-flex items-center justify-center gap-2 mt-6 rounded-lg bg-energy px-6 py-3 text-sm font-semibold text-white transition-all hover:bg-energy-light"
          >
        ...
        <div className="mx-auto max-w-2xl flex flex-col gap-4">
          ...
          {result.deckScore && (
            <details className="rounded-xl border border-tan-200 bg-tan-100 p-5 backdrop-blur-sm group">
              <summary className="flex items-center justify-between cursor-pointer list-none [&::-webkit-details-marker]:hidden">
                ...
                <svg className="w-4 h-4 text-brown-400 transition-transform group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="rounded-xl border border-tan-200 bg-tan-100 p-5 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-3 divide-x divide-tan-200">
              <div className="pr-4">
                <p className="text-xs text-brown-400 uppercase tracking-wide mb-1">Pok&eacute;mon</p>
          ...
          <div className="text-center mt-4">
            <Link
              href="/deck-profiler"
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-energy px-6 py-3 text-sm font-semibold text-white transition-all hover:bg-energy-light"
            >
              Profile your own deck
            </Link>
          </div>
